    ...
    internal class Solution : Solution<(bool, (int, int, int), (int, int, int))[]>
    {
                ...
                for (int x = instruction.Item2.Item1; x <= instruction.Item3.Item1; x++)
                {
                    for (int y = instruction.Item2.Item2; y <= instruction.Item3.Item2; y++)
                    {
                        for (int z = instruction.Item2.Item3; z <= instruction.Item3.Item3; z++)
                        {
                            try
                            {
                                this.SetValue(instruction.Item1, x, y, z, ref reactor);
                            }
        ...
        exit:
            ...
            for (int x = -50; x <= 50; x++)
            {
                for (int y = -50; y <= 50; y++)
                {
                    for (int z = -50; z <= 50; z++)
                    {
                        if (this.GetValue(x, y, z, reactor))
                        {
                            activeCubes++;
                        }
                    }
        ...
        internal override (object, string) Puzzle2((bool, (int, int, int), (int, int, int))[] input)
        {
            ...
            for (int i = input.Length - 1; i >= 0; i--)
            {
                ...
                long volume = this.UniqueIntersection((instruction.Item2, instruction.Item3), reactor);
        ...
        {
            long volume = this.Volume(intersection);
            ...
            foreach (var cube in toIntersect)
            {
                try
                {
                    intersections.Enqueue(this.Intersect(intersection, cube));
                }
                catch (IndexOutOfRangeException)
                {
                }
            ...
            {
                volume -= this.UniqueIntersection(intersections.Dequeue(), intersections.ToList());
            }

            if (intersections.Count > 0)
            {
                volume -= this.Volume(intersections.Dequeue());
            }
        ...
        {
            return this.Volume((instruction.Item2, instruction.Item3));
        }
        ...
        {
            if (value == reactor[HalfMax + x, HalfMax + y, HalfMax + z])
            {
                return false;
            }
